...
\exp\left[-\frac{1}{2\sigma_i^2}\left(D_i - \mu(t_i; A, b, t_c, w)\right)^2\right]\label{eq:lgaussian}
\end{eqnarray}
...
function of the parameters called the likelihood function.
...
\begin{eqnarray}
D_i \sim \mathcal{N}\left(\mu(t_i; A, b, t_c, w), \sigma_i^2\right)
\end{eqnarray}
...
second is how to calculate the results efficiently.
...
Nested Sampling is a Monte Carlo algorithm introduced by \citet{skilling2006nested}.
...
algorithms, while all based on the insights of \citet{skilling2006nested}, are very
