...
this chapter are provided online at
{\tt http://www.iac.es/winterschool/2014/pages/teaching-material.php}.
...
\begin{minted}[mathescape,
  ...
  def log_likelihood(params):
    ...
    """
    # Rename the parameters
    A, b, tc, width, log_nu, K = params[0], params[1], params[2],\
                                 params[3], params[4]
    ...
    return scipy.special.gammaln(0.5*(nu+1.))\
         - scipy.special.gammaln(0.5*nu)\
         - np.sum(np.log(sig*np.sqrt(nu)))\
         - 0.5*(nu + 1.)*np.log(1. + (data[:,1] - mu)**2/nu/sig**2)
\end{minted}
...
Consider a function that takes the best fit (highest likelihood) point in the
parameter space and maps it to $X=0$. 
...
%logZ = -161.038664757
%Information = 15.1328174099 nats
