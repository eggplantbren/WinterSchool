...
\begin{minted}[mathescape,
  ...
  # In the full version of the code, the initial point is drawn from the prior.
...
directly accessible since we parameterised things by $\log(\nu)$ and $u_K$
instead. However, since these both had uniform priors, their marginal posteriors
...
parameter space is mapped to $X=0$ (modulo mathemetical technicalities),
...
\end{eqnarray}
$X$ can also be considered a function of a likelihood value:
\begin{eqnarray}
